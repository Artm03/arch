...
from db import mongo as mongo_utils
...
async def get_products(
    ...
    limit: int = 50,
    offset: int = 0,
    collection: collection.AsyncCollection = fastapi.Depends(mongo_utils.get_mongo_collection)
):
    filter_query = {}
    ...
    if min_price is not None:
        price_filter['$gte'] = mongo_utils.decimal_to_decimal128(decimal.Decimal(str(min_price)))
    if max_price is not None:
        price_filter['$lte'] = mongo_utils.decimal_to_decimal128(decimal.Decimal(str(max_price)))
    if price_filter:
        filter_query['price'] = price_filter
    
    products: tp.List[product_models.Product] = []
    async for item in collection.find(filter_query).skip(offset).limit(limit):
        products.append(mongo_utils.mongo_to_product(item))
...
async def get_product(product_id: str, collection: collection.AsyncCollection = fastapi.Depends(mongo_utils.get_mongo_collection)):
    try:
    ...
    return mongo_utils.mongo_to_product(product_doc)
...
async def create_product(
    ...
    product_dict['price'] = mongo_utils.decimal_to_decimal128(product_dict['price'])

    result = await collection.insert_one(product_dict)
    created_product = await collection.find_one({"_id": result.inserted_id})
    
    return mongo_utils.mongo_to_product(created_product)
...
async def update_product(
    ...
    update_data['price'] = mongo_utils.decimal_to_decimal128(update_data['price'])
    ...
    return mongo_utils.mongo_to_product(updated_product)


# DELETE /products/delete - Удалить продукт по ID
@router.delete("/delete", response_model=product_models.Product)
async def delete_product(product_id: str, collection: collection.AsyncCollection = fastapi.Depends(mongo_utils.get_mongo_collection)):
    try:
        object_id = bson.ObjectId(product_id)
    except Exception:
        raise fastapi.HTTPException(status_code=404, detail="Invalid product ID format")
...
async def create_products_bulk(
    ...
    if product_dicts:
        ...
        async for item in collection.find({"_id": {"$in": result.inserted_ids}}):
            inserted_products.append(mongo_utils.mongo_to_product(item))
